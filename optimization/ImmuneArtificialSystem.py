...
min_features = [-512, -512]
max_features = [512, 512]
...
class AIRS:
    ...
    def calculate_affinity_threshold(self):
        ...
        for i in range(len(train_set)):
            for j in range(i + 1, len(train_set)):
                affinity_threshold += self.affinity(
                    train_set[i][0], train_set[j][0])

        self.AFFINITY_THRESHOLD = affinity_threshold / \
            (len(train_set) * (len(train_set) - 1) / 2)
    ...
    def init_MC(self, train_set, MC):
        ...
        for _ in range(int(len(train_set) * self.MC_INIT_RATE)):
            ...
            MC[seed_cell[1]].append(
                Cell(vector=seed_cell[0], _class=seed_cell[1]))
    ...
    def train(self):
        ...
        for antigene, _class in train_set:
            ...
            # ARB Generation
            # add the mc_match to ARBs
            AB[_class].append(
                ARB(vector=mc_match.vector, _class=mc_match._class))
            stim = mc_match.stimulate(antigene)

            iterations = 0
            while True:
                iterations += 1
                MAX_CLONES = int(self.HYPER_CLONAL_RATE *
                                 self.CLONAL_RATE * stim)
                num_clones = 0
                ...
                avgStim = sum([x.stimulate(antigene)
                              for x in AB[_class]]) / len(AB[_class])
                ...
                for c in AB.keys():
                    for ab in AB.get(c):
                        ab.stimulation = (
                            ab.stimulation - MIN_STIM) / (MAX_STIM - MIN_STIM)
                ...
                while resAlloc > numResAllowed:
                    ...
                    abRemove, abRemoveIndex = self.argminARB(
                        AB=AB, _class=_class)
    ...
    def classify(self, antigene):
        ...
        vote_array = list(
            sorted(vote_array, key=lambda cell: -cell.stimulation))
        v = {0: 0, 1: 0, 2: 0}
...
class ARB:
    ...
    def stimulate(self, pattern):
        self.stimulation = 1 - \
            AIRS.affinity(vector1=pattern, vector2=self.vector)
